...
def user_input_features():
    ...
    Sleep_Duration = st.sidebar.slider(
        "Sleep Duration", min_value=5.5, max_value=9.0, step=0.1
    )
    Quality_of_Sleep = st.sidebar.slider(
        "Quality of Sleep", min_value=4, max_value=9, step=1
    )
    Physical_Activity_Level = st.sidebar.slider(
        "Physical Activity Level", min_value=30, max_value=90, step=1
    )
...
st.markdown(input_df.to_markdown())

st.subheader("Prediction")


if prediction[0] == "Normal":
    st.success("You're Normal Keep up the Routine")

elif prediction[0] == "Insomnia":
    st.error(
        "You Have Symptoms of Insomnia Kindly takecare of your Health Please do visit the Clinician"
    )

elif prediction[0] == "Sleep Apnea":
    st.warning(
        "You have Symptoms of Sleep Apnea Kindly takecare of your Health Please do visit the Clinician"
    )
else:
    st.exception("Try after Some time..")
